    ...
    XMFLOAT3 cameraPosition, XMFLOAT4 specularColor, float specularPower)
{
    ...
    res = SetShaderParameters(deviceContext, worldMatrix, viewMatrix, projectionMatrix, texture, lightDirection, ambientColor, diffuseColor, cameraPosition, specularColor, specularPower);    
    if (!res)
    {
        return false;
    }
...
bool LightShader::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
    ...
    // Buffer description for camera buffer
    D3D11_BUFFER_DESC cameraBufferDesc;
    // Buffer description for light buffer
    ...
    res = D3DCompileFromFile(vsFilename, NULL, NULL, "LightVertexShader", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, &vertexShaderBuffer, &errorMessage);
    if (FAILED(res))
    {
    ...
    res = D3DCompileFromFile(psFilename, NULL, NULL, "LightPixelShader", "ps_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, &pixelShaderBuffer, &errorMessage);
    if (FAILED(res))
    {
        // If the shader failed to compile it should have writen something to the error message.
        if (errorMessage)
    ...
    res = device->CreateBuffer(&cameraBufferDesc, NULL, &m_cameraBuffer);
    if (FAILED(res))
    {
        return false;
    }
    ...
    // Create the constant buffer pointer so we can access the vertex shader constant buffer from within this class.
    res = device->CreateBuffer(&lightBufferDesc, NULL, &m_lightBuffer);
    if (FAILED(res))
    {
        return false;
    }
